...
def get_volume_and_number(slug):
    for i, (s, t, desc) in enumerate(lessons):
        if s == slug:
            idx = i + 1
            if idx <= 6:
                return ('一', str(idx).zfill(2))
            elif idx <= 13:
                return ('二', str(idx).zfill(2))
            elif idx <= 18:
                return ('三', str(idx).zfill(2))
            elif idx <= 30:
                return ('四', str(idx).zfill(2))
            else:
                return ('五', '附' + str(idx - 30))
    return ('一', '01')

# 获取下一课
def get_next_lesson(slug):
    for i, (s, t, desc) in enumerate(lessons):
        if s == slug and i < len(lessons) - 1:
            return lessons[i+1]
    return None
...
for idx, (slug, title, subtitle) in enumerate(lessons):
    ...
    try:
        ...
        # 简单提取 lesson-body 部分
        body_start = html.find('<main class="lesson-body"')
        body_end = html.find('</main>', body_start + 10)
        ...
        if body_start > 0 and body_end > 0:
            ...
            # 获取下一课
            next_lesson = get_next_lesson(slug)
            next_html = ''
            if next_lesson:
                next_slug, next_title, next_sub = next_lesson
                next_html = f'''<a href="{next_slug}.html" class="next-card">
                    <span class="next-arrow">→</span>
                    <span class="next-label">NEXT · 第 {str(idx+2).zfill(2)} 课</span>
                    <h3>{next_title}</h3>
                    <p>{next_sub}</p>
                </a>'''
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* 清理原站不需要的元素 */
        .like-btn, .like-btn-wrapper {{ display: none !important; }}
